...
void myInitDelay(){
	uint16_t* AutoReload = (uint16_t*)(myTIM6 + myCR1);
	*AutoReload  |= 1 << 7 | 1 << 2 | 1 << 3;
	*AutoReload  &= ~(1<<1);
	uint16_t* Prescaler = (uint16_t*)(myTIM6 + myPSC);
	*Prescaler = 4000;
}

void myDelay(uint16_t time){
	uint16_t* AutoReloadValue = (uint16_t*)(myTIM6 + myARR);
	*AutoReloadValue = time;

	uint16_t* EGR = (uint16_t*)(myTIM6 + myEGR);
	*EGR  |= 1 << 0;

	uint16_t* PowerCounter = (uint16_t*)(myTIM6 + myCR1);
	*PowerCounter  |= 1 << 0;

	uint32_t* Counter = (uint32_t*)(myTIM6 + myCNT);
	*Counter = 0;

	uint16_t* UIE = (uint16_t*)(myTIM6 + myDIER);
	*UIE  |= 1 << 0;

    uint16_t* StatusFlag = (uint16_t*)(myTIM6 + mySR);
    while(!(*StatusFlag & 1)){
    }
    *StatusFlag  &= ~(1 << 0);
}
...
void myClockInit(void){
    ...
    *SYSCFGClock |= 1 << 0;
    ...
    uint32_t* PowerTIM6= (uint32_t*)(myRCC + myAPB1ENR1);
    *PowerTIM6  |= 1 << 4;
}
